...
# Reads remote data from server/pod/file_name and write them to local_folder/file_name
def read_inverse(local_folder, file_name):
...
# Reads remote data from server/pod/file_name and write them to local_folder/file_name
def read_data_from_pod_inverseMonth(local_folder, file_name):
    remote_url = f"{base_url}/{file_name}"
...
def url_exists(pod_folder):
    try:
        remote_url = f"{base_url}/{pod_folder}/"
        print(remote_url)
        response = requests.head(remote_url, timeout=5)
        # Se il codice di stato è 200 (OK), allora l'URL esiste
        return response.status_code == 200
    except requests.RequestException:
        # Se si verifica un'eccezione (es. URL non raggiungibile), restituiamo False
        return False
